...
static bool tm_to_time_point(const tm &_tm, std::chrono::time_point<std::chrono::system_clock> &tp)
{
	...
	// Use safer localtime_r or localtime_s depending on platform
	tm localResult;
	tm utcResult;

#ifdef _WIN32
	// Windows version: use localtime_s and gmtime_s
	if (localtime_s(&localResult, &t) != 0 || gmtime_s(&utcResult, &t) != 0)
		return false;
#else
	// POSIX version: use localtime_r and gmtime_r
	if (localtime_r(&t, &localResult) == nullptr || gmtime_r(&t, &utcResult) == nullptr)
		return false;
#endif

	// Adjust for timezone differences between local and UTC
	t += mktime(&localResult) - mktime(&utcResult);
